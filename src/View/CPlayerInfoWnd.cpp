...
void CPlayerInfoWnd::update()
{
   ...
   for (int i = 0; i<BUTTON_COUNT; i++)
	{
		if (i == 0)
		{
         int itemID = m_pPlayer->getEquip(SHOULDER);
         if(itemID == -1)
		...
		{
         int itemID = m_pPlayer->getEquip(GLOVE);
         if(itemID == -1)
            m_vpBtn[i]->setImage("Examples/ogreborder");
         else {
            CItemInfo *pItemInfo = CItem::getInfo(itemID);
		...
		{
         int itemID = m_pPlayer->getEquip(MAIN_HAND);
         if(itemID == -1)
            m_vpBtn[i]->setImage("Examples/ogreborder");
         else {
            CItemInfo *pItemInfo = CItem::getInfo(itemID);
		...
		{
         int itemID = m_pPlayer->getEquip(CHEST);
         if(itemID == -1)
            m_vpBtn[i]->setImage("Examples/ogreborder");
         else {
            CItemInfo *pItemInfo = CItem::getInfo(itemID);
		...
		{
         int itemID = m_pPlayer->getEquip(LEGS);
         if(itemID == -1)
            m_vpBtn[i]->setImage("Examples/ogreborder");
         else {
            CItemInfo *pItemInfo = CItem::getInfo(itemID);
		...
		{
         int itemID = m_pPlayer->getEquip(OFF_HAND);
         if(itemID == -1)
            m_vpBtn[i]->setImage("Examples/ogreborder");
         else {
            CItemInfo *pItemInfo = CItem::getInfo(itemID);
		...
		{
         int itemID = m_pPlayer->getEquip(BOOT);
         if(itemID == -1)
            m_vpBtn[i]->setImage("Examples/ogreborder");
         else {
            CItemInfo *pItemInfo = CItem::getInfo(itemID);
   ...
   //更新基本資訊
	for (int i = 1; i<3; i++)
	{
		...
		if (i == 1)
		{
         m_vpText[i]->setText (m_pPlayer->getName(), 1, 1, 1) ;
		}else if (i == 2)
		{
         sprintf_s (buf, sizeof (buf), "等級 %3d/經驗值 %3d", m_pPlayer->getLevel(), m_pPlayer->getXP()) ;
			m_vpText[i]->setText (buf, 1, 1, 1) ;
		}
	}

   // 更新屬性資料
   AdvancedAttribute advAttr = m_pPlayer->getAdvAttr();
	for (int i = 3; i<TEXT_COUNT; i++)
	{
		char buf[256] ;		
		if (i == 3)
